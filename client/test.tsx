...
import {
  IFriendGroupItem,
  IGroupChatInfo,
} from '@/components/CreateGroupChatModal/type'
...
const AddressBook = forwardRef((props: IAddressBookProps, ref) => {
  ...
  const treeData = friendList.map((group) => {
    return {
      title: (
        <span>
          {group.name}&nbsp;&nbsp;&nbsp;&nbsp;{group.online_counts}/
          {group.friend.length}
        </span>
      ),
            ...
            {curGroupChatInfo?.members.map((item) => {
              return (
                <div className={styles.tableItem} key={item.user_id}>
                  ...
                  <li>
                    {item.lastMessageTime?.split('.')[0].replace('T', '') ||
                      '暂无'}
                  </li>
                </div>
              )
              ...
              return (
                <div
                  className={`${styles.groupChat} ${
                    curGroupChatInfo?.id === item.id
                      ? styles.curGroupChatInfo
                      : ''
                  }`}
                  key={item.id}
            ...
            <div className={styles.infoModal}>
              <div className={styles.infoContainerHead}>
                ...
                <div className={styles.info}>
                  ...
                  <div
                    className={`icon iconfont icon-gexingqianming ${styles.signature}`}
                  >
            ...
            <div className={styles.infoModal}>
              <div className={styles.infoContainerHead}>
                ...
                <div className={styles.info}>
                  ...
                  <div
                    className={`icon iconfont icon-qungonggao ${styles.signature}`}
                  >
                    {curGroupChatInfo.announcement
                      ? curGroupChatInfo.announcement
                      : '暂无公告'}
                  </div>
                </div>
              ...
              <div className={styles.btns}>
                <Button onClick={() => handleCreateModal(true)}>
                  {' '}
                  邀请好友{' '}
                </Button>
                <Button
